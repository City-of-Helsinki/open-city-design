import React, { Component } from 'react';
import { Button, Grid, Jumbotron, Col, Row, ButtonToolbar, Nav, Navbar, NavDropdown, NavItem, MenuItem } from 'react-bootstrap';
import './Home.css';
import Sidebar from '../components/Sidebar';
import Welcome from '../components/Welcome';


class Development extends Component {
  render() {
      const imgStyle = {
            maxHeight: '20px',
      };
      const welcome = {
            heading: "Welcome Developer",
            paragraph: "Use this guide to develop"
      };
      const sidebarLinks = [
            { link: "Components", URL: 'Components' },
            { link: "Modules", URL: 'Modules' },
            { link: "Patterns", URL: 'Patterns' },
            { link: "Page Templates", URL: 'PageTemplates' },
      ];
      
    return (
          
      <div>
           <Col xs={6} md={4}>
                 <Sidebar sidebarLinks={sidebarLinks}/>
           </Col>
           <Col xs={6} md={8}>
           <Welcome welcome={welcome}/>

           <Button>Components<img style={imgStyle} src="https://camo.githubusercontent." /></Button>
            <Button>Modules</Button>
            <Button>Patterns</Button>
            <Button>Page templates</Button>

           </Col>
      </div>

    );
  }
}
export default Development;

